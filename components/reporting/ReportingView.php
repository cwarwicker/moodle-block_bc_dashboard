...
class ReportingView extends \block_bc_dashboard\View {
    ...
    public static function buildSideNav() {
        ...
        // Public reports
        $sideNav[] = array( 'title' => get_string('publicreports', 'block_bc_dashboard'), 'icon' => 'fa-folder|fa-folder-open', 'children' => self::createSideNav(\block_bc_dashboard\Report::pub()) );

        // Private reports
        $privReports = \block_bc_dashboard\Report::priv();
        $children = array();
        if ($privReports) {
        ...
        $myReports = \block_bc_dashboard\Report::my();
        $children = array();
        if ($myReports) {
