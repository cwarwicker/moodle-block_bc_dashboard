...
class Config {
    ...
    protected function getRecursiveReportCategories($reports = false, $parent = null, $includeUncategorised = true) {
        ...
        if ($records) {
            foreach ($records as $record) {
                ...
                // Load reports
                if ($reports) {
                    $record->reports = \block_bc_dashboard\Report::pub($record->id);
                }
        ...
        if (is_null($parent) && $includeUncategorised) {
            ...
            $misc->reports = \block_bc_dashboard\Report::pub(0);
            $return[] = $misc;
        }
    ...
    public function loadPostData() {
        ...
        $settings = array(
            'report_cat' => df_optional_param_array_recursive('report_cat', false, PARAM_TEXT),
            'course_cats' => df_optional_param_array_recursive('course_cats', false, PARAM_TEXT),
            'elements_enabled' => df_optional_param_array_recursive('elements_enabled', false, PARAM_TEXT),
        );

        // Report categories
        $cats = array();

        if ($settings['report_cat']) {
            foreach ($settings['report_cat'] as $key => $cat) {
                $obj = new \stdClass();
        ...
        // Course categories
        $this->courseCats = ($settings['course_cats']) ? $settings['course_cats'] : array();

        // Reporting elements
        $this->reportingElements = ($settings['elements_enabled']) ? $settings['elements_enabled'] : array();
    ...
    public function save() {
        ...
        \block_bc_dashboard\Setting::updateSetting("course_cats", $courseCats);

        // Reporting elements
        $elements = \block_bc_dashboard\Report\Element::retrieve(false, true);
        if ($elements) {
            foreach ($elements as $id => $element) {
                $obj = new \stdClass();
                $obj->id = $element->getID();
                $obj->enabled = (array_key_exists($id, $this->reportingElements)) ? 1 : 0;
                \block_bc_dashboard\Report\Element::update($obj);
            }
        }
    ...
    public function getTopLevelCourseCategories() {
        ...
        $courseCats = explode(",", \block_bc_dashboard\Setting::getSetting('course_cats'));
        if ($courseCats) {
