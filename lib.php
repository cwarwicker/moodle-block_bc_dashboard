...
function bcdb_callHook($url = null){
    ...
    if (is_null($url) || empty($url)){
        $Control = new \BCDB\Controllers\IndexController();
    }
    else
    {
        ...
        for($i = 0; $i < $cnt; $i++)
        {
            ...
            $Info['controller_class'] = "\\BCDB\\Controllers\\" . $implodeClass . ucfirst($el) . 'Controller';
        ...
        if (!$Control)
        {
            ...
            $index = new \BCDB\Controllers\IndexController(true);
...
//function bcdb_get_courses(){
//    
//    global $DB;
//    
//    $results = array();
//    $records = $DB->get_records("course", array(), "fullname, shortname", "id, shortname, fullname");
//                             
//    if ($records)
//    {
...
function bcdb_get_gradetracker_quals(){
    ...
    $quals = \GT\Qualification::getAllQualifications(true);
    $listOfQuals = array();
    if ($quals)
    {
...
function bcdb_confirmation_page($title, $output, $url = null, $timeout = null, $viewObj = null){
    ...
    $View = (!is_null($viewObj)) ? $viewObj : new BCDB\Views\IndexView();
    $View->setController(new BCDB\Controllers\IndexController(true));
...
function bcdb_fatalError($msg){
    ...
    if (isset($_GET['Qs']) && strpos($_GET['Qs'], "error") === 0){
        $explode = explode("/", $_GET['Qs']);
        $vars['msg'] = $explode[1];
    }
...
function bcdb_get_full_category_name($id, &$names = array()){
    ...
    if (!$record) return get_string('uncategorised', 'block_bc_dashboard');
